...
## exactly that: Use previous calculated matrix invertion.

## The two functions are: makeCacheMatrix and cacheSolve. The first creates
## a sepecial type of matrix that his a list and have some public methods.
## The second function use the special matrix list created with the 
## first function to avoid repeating the computation if data is the same
...
## If the inverse matix is already calculated, just gets the previous
## calculation

cacheSolve <- function(x, ...) { ## argument is a makeCacheMatrix object
  
  m <- x$getinverse() ## get the inverse matrix from makeCacheMatrix
  ## object
